...
const TEST_SAVE_PATH := "user://issue_64_first_ramp_profile.json"
...
func _initialize() -> void:
	...
	await _verify_level_1_freehand_physics()
	if _failed:
		return

	print("Issue #64 First Ramp verification passed: Level 1 renders freehand physics, touch strokes create visible collision, short/flat lines fail, reset clears runtime state, and a broad rising ramp reaches the cup by overlap.")
	_cleanup()
	quit(0)
...
func _verify_level_1_freehand_physics() -> void:
	var level := _level_by_number(1)
	var level_id := str(level.get("id", ""))
	_require(str(level.get("title", "")) == "First Ramp", "Level 1 should be titled First Ramp.")
	_require(str(level.get("template", "")) == "Physics Draw", "Level 1 should render as Physics Draw.")
	_require(str(_dictionary_from(level.get("rules", {})).get("interaction_model", "")) == "freehand_physics_then_release", "Level 1 should use freehand physics.")
	if _failed:
		return

	_main.call("_show_play_screen", level)
	await process_frame
	_assert_first_ramp_surface()
	if _failed:
		return

	_draw_line_with_touch(Vector2(52, 226), Vector2(82, 214))
	_require(str(_main.get("_last_physics_result")) == "too_short", "Too-short touch stroke should fail before release.")
	_press_button("Release")
	_require(str(_main.get("_last_physics_result")) == "fail", "Too-short stroke should fail on release.")
	_require(_screen_has_label_text("Too short"), "Too-short failure should be readable.")
	_require(not _profile.is_level_completed(level_id), "Too-short stroke should not complete Level 1.")
	...
	_draw_line_with_touch(Vector2(42, 232), Vector2(288, 232))
	_require(_stroke_collision_count() > 0, "Long flat stroke should create collision geometry before release.")
	_press_button("Release")
	_require(str(_main.get("_last_physics_result")) == "fail", "Bad flat stroke should fail on release.")
	_require(_screen_has_label_text("Too flat"), "Bad flat failure should be readable.")
	_require(not _profile.is_level_completed(level_id), "Bad flat stroke should not complete Level 1.")
	...
	_require(line != null and line.points.size() >= 2, "Touch stroke should create visible line points.")
	_require(_stroke_collision_count() > 0, "Valid ramp stroke should create StaticBody2D collision shapes.")
	...
	_require(str(_main.get("_last_physics_result")) == "success", "Broad rising ramp should succeed by ball/cup overlap.")
	_require(_profile.is_level_completed(level_id), "Broad rising ramp should complete Level 1.")
	_require(_screen_has_label_text("Score Roastcard"), "Valid ramp should route to Score Roastcard.")
...
func _assert_first_ramp_surface() -> void:
	_require(_node_named(_main, "physics_draw_surface") != null, "Level 1 should render a named draw surface.")
	_require(_node_named(_main, "freehand_ball") != null, "Level 1 should render a freehand ball.")
	_require(_node_named(_main, "freehand_cup") != null, "Level 1 should render a freehand cup.")
	_require(_node_named(_main, "freehand_goal_area") is Area2D, "Level 1 should create an Area2D goal zone.")
	_require(_button_with_text(_main, "Release") != null, "Level 1 should expose Release.")
	_require(_button_with_text(_main, "Reset") != null, "Level 1 should expose Reset.")
	_require(_button_with_text(_main, "Release Ball") == null, "Level 1 should not use the old Release Ball copy.")
	_require(not _has_button_prefix(_main, "Draw:"), "Level 1 should not expose answer-choice Draw buttons.")
	_require(_node_named(_main, "tap_scene_surface") == null, "Level 1 should no longer render Tap Logic UI.")
...
func _assert_freehand_reset_state() -> void:
	...
	_require(ball_position is Vector2 and (ball_position as Vector2).distance_to(Vector2(58, 218)) < 0.1, "Reset should return the ball to its start.")
