...
const TEST_SAVE_PATH := "user://issue_10_pack_2_smoke_profile.json"
...
func _initialize() -> void:
	...
	if _failed:
		return
	_assert_save_load_state("Pack 2 completion", _all_levels)
	if _failed:
		return

	_run_level_20_replay_best_attempt_checks()
	if _failed:
		return
	_assert_save_load_state("Level 20 replay", _all_levels)
	if _failed:
		return
	...
	print("Issue #10 Pack 2 smoke passed: %d Level List rows, %d completion(s), %d Pack 2 completion(s), %d replay(s), %d Score Roastcard(s), %d save/load check(s), %d restart check(s), %d Dur spend(s), %d Dur recovery event(s)." % [
		TOTAL_LEVEL_COUNT,
		_completion_count,
...
func _load_combined_levels() -> void:
	...
	var pack_1 := loader.load_pack(PACK_1_PATH)
	if not _require(not pack_1.is_empty(), loader.last_error):
		return

	var pack_2 := loader.load_pack(PACK_2_PATH)
	if not _require(not pack_2.is_empty(), loader.last_error):
		return

	if not _require(str(pack_1.get("pack_id", "")) == PACK_1_ID, "Pack 1 pack_id should be %s." % PACK_1_ID):
		return
	if not _require(str(pack_2.get("pack_id", "")) == PACK_2_ID, "Pack 2 pack_id should be %s." % PACK_2_ID):
		return

	_all_levels = []
	_pack_2_levels = []
	for level_number in range(FIRST_LEVEL_NUMBER, PACK_2_LAST_LEVEL + 1):
		var source_pack := pack_1
		if level_number >= PACK_2_FIRST_LEVEL:
			source_pack = pack_2

		var level := loader.find_level_by_number(source_pack, level_number)
	...
	if not _require(_all_levels.size() == TOTAL_LEVEL_COUNT, "Issue #10 smoke should load exactly %d Level Specs." % TOTAL_LEVEL_COUNT):
		return
	if not _require(_pack_2_levels.size() == 10, "Issue #10 smoke should load exactly 10 Pack 2 Level Specs."):
		return

	_combined_pack = {
		"pack_id": "issue_10_pack_1_and_2_smoke",
		"source_path": "%s + %s" % [PACK_1_PATH, PACK_2_PATH],
		"levels": _all_levels,
	}
	_inject_combined_pack()
...
func _assert_clean_profile() -> void:
	if not _require(_profile.current_uqiq_score() == LocalProfileScript.DEFAULT_UQIQ_SCORE, "Clean Pack 2 smoke profile should start at UQIQ 100."):
		return
	if not _require(_profile.dur_tokens() == LocalProfileScript.MAX_DUR_TOKENS, "Clean Pack 2 smoke profile should start with 3 Dur Tokens."):
		return
	if not _require(_profile.is_level_unlocked(1), "Clean Pack 2 smoke profile should start with Level 1 unlocked."):
		return
	if not _require(not _profile.is_level_unlocked(2), "Clean Pack 2 smoke profile should start with Level 2 locked."):
		return
	if not _require(not _profile.is_level_unlocked(PACK_2_FIRST_LEVEL), "Clean Pack 2 smoke profile should start with Level 11 locked."):
		return
...
func _run_pack_1_unlock_flow() -> void:
	...
	_spend_dur_from_level_list(level_10, "DUR'ing Level 10 should unlock Level 11.", 11)
	if _failed:
		return
...
func _run_pack_2_completion_flow() -> void:
	for level in _pack_2_levels:
		...
		if level_number < PACK_2_LAST_LEVEL:
			...
			if not _require(int(_profile.data.get("unlocked_level", 0)) >= PACK_2_LAST_LEVEL + 1, "Completing Level 20 should advance Local Profile beyond Pack 2."):
				return


func _run_level_20_replay_best_attempt_checks() -> void:
	var level := _level_by_number(PACK_2_LAST_LEVEL)
	var level_id := str(level.get("id", ""))

	var first_best: Dictionary = _profile.get_best_attempt(level_id)
	if not _require(not first_best.is_empty(), "Level 20 should have a Best Attempt before replay."):
		return
	if not _require(int(first_best.get("roast_count", -1)) > 0, "Initial Level 20 completion should include Roast usage so replay can improve it."):
		return
	...
	if not _require(int(better_best.get("roast_count", -1)) == 0, "Better Level 20 replay should improve Best Attempt Roast count."):
		return
	if not _require(int(better_best.get("action_count", 9999)) <= int(first_best.get("action_count", 9999)), "Better Level 20 replay should keep or improve Best Attempt action count."):
		return
	...
	if not _require(int(kept_best.get("roast_count", -1)) == 0, "Worse Level 20 replay should keep the improved Best Attempt Roast count."):
		return
	if not _require(int(kept_best.get("action_count", 9999)) == int(better_best.get("action_count", 9999)), "Worse Level 20 replay should keep the improved Best Attempt action count."):
		return
...
func _complete_level_by_template(level: Dictionary) -> void:
	match str(level.get("template", "")):
		...
		_:
			if not _require(false, "Unsupported Level Template in Pack 2 smoke: %s" % str(level.get("template", ""))):
				return
...
func _assert_restart_state() -> void:
	...
	if not _require(_profile.current_uqiq_score() == expected_score, "App restart should preserve UQIQ Score after Pack 2."):
		return
	if not _require(_profile.dur_tokens() == expected_tokens, "App restart should preserve Dur Tokens after Pack 2."):
		return
	if not _require(int(_profile.data.get("unlocked_level", 0)) == expected_unlocked, "App restart should preserve Level 20 unlock progress."):
		return
...
func _level_by_number(level_number: int) -> Dictionary:
	...
	_require(false, "Level %d should be loaded for Issue #10 smoke." % level_number)
	return {}
