...
func _verify_tap_fail_roast_success_feedback() -> void:
	...
	_require(_feedback_count("tap") == 1, "Wrong tap should fire tap feedback.")
	_require(_feedback_count("fail") == 1, "Wrong tap should fire fail feedback.")
	_require(int(_main.get("_tap_count")) == 1, "Feedback should not add extra actions after wrong tap.")

	_main.call("_handle_roast_action")
	_require(_feedback_count("roast") == 1, "Roast action should fire Roast feedback.")
	_require(int(_main.get("_tap_count")) == 1, "Roast feedback should not change action count.")
	...
	_require(_feedback_count("tap") == 2, "Correct tap should fire tap feedback.")
