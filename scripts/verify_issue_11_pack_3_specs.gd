...
const PACK_PATH := "res://content/levels/pack_03_move_the_wrong_thing.json"
const EXPECTED_PACK_ID := "pack_03_move_the_wrong_thing"
const EXPECTED_LEVEL_COUNT := 10
const FIRST_LEVEL_NUMBER := 21
...
func _initialize() -> void:
	if not FileAccess.file_exists(PACK_PATH):
		_fail("Missing Pack 3 Level File: %s" % PACK_PATH)
		_finish()
		return

	var file := FileAccess.open(PACK_PATH, FileAccess.READ)
	if file == null:
		_fail("Could not open Pack 3 Level File: %s" % PACK_PATH)
		_finish()
		return

	var parsed = JSON.parse_string(file.get_as_text())
	if typeof(parsed) != TYPE_DICTIONARY:
		_fail("Pack 3 Level File must be a JSON object: %s" % PACK_PATH)
		_finish()
		return
...
func _validate_pack(pack: Dictionary) -> void:
	...
	if not pack.has("pack_id"):
		_fail("Pack 3 file missing top-level pack_id.")
	elif str(pack.get("pack_id", "")).strip_edges() != EXPECTED_PACK_ID:
		_fail("Pack 3 pack_id must be %s." % EXPECTED_PACK_ID)

	var levels = pack.get("levels", [])
	if typeof(levels) != TYPE_ARRAY:
		_fail("Pack 3 file must contain a levels array.")
		return

	if levels.size() != EXPECTED_LEVEL_COUNT:
		_fail("Pack 3 must contain exactly %d Level Specs, found %d." % [EXPECTED_LEVEL_COUNT, levels.size()])
...
func _finish() -> void:
	if _errors.is_empty():
		print("Issue #11 Pack 3 spec verification passed: %d Level Specs, %s, Levels %d-%d, supported templates, scoring, Roasts, unique ids, and UQIQ Moments." % [
	...
	for message in _errors:
		push_error(message)
	print("Issue #11 Pack 3 spec verification failed: %d error(s)." % _errors.size())
	quit(1)
