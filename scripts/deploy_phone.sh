...
PLAYTEST_UNLOCK_ALL="${UQIQ_PLAYTEST_UNLOCK_ALL:-}"
...
install_launch_capture() {
	...
	if [[ -z "$LAUNCH_ENV_JSON" && -n "$PLAYTEST_UNLOCK_ALL" ]]; then
		LAUNCH_ENV_JSON="{\"UQIQ_PLAYTEST_UNLOCK_ALL\":\"$PLAYTEST_UNLOCK_ALL\"}"
	fi
