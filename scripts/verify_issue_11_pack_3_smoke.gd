...
const TEST_SAVE_PATH := "user://issue_11_pack_3_smoke_profile.json"
...
func _initialize() -> void:
	...
	if _failed:
		return
	_assert_save_load_state("Pack 3 completion", _all_levels)
	if _failed:
		return

	_run_level_30_replay_best_attempt_checks()
	if _failed:
		return
	_assert_save_load_state("Level 30 replay", _all_levels)
	if _failed:
		return
...
func _load_combined_levels() -> void:
	...
	for level_number in range(FIRST_LEVEL_NUMBER, PACK_3_LAST_LEVEL + 1):
		...
		elif level_number >= PACK_3_FIRST_LEVEL:
			_pack_3_levels.append(level)

	if not _require(_all_levels.size() == TOTAL_LEVEL_COUNT, "Issue #11 smoke should load exactly %d Level Specs." % TOTAL_LEVEL_COUNT):
		return
	if not _require(_pack_2_levels.size() == 10, "Issue #11 smoke should load exactly 10 Pack 2 Level Specs."):
		return
...
func _assert_clean_profile() -> void:
	if not _require(_profile.current_uqiq_score() == LocalProfileScript.DEFAULT_UQIQ_SCORE, "Clean Pack 3 smoke profile should start at UQIQ 100."):
		return
	if not _require(_profile.dur_tokens() == LocalProfileScript.MAX_DUR_TOKENS, "Clean Pack 3 smoke profile should start with 3 Dur Tokens."):
		return
	if not _require(_profile.is_level_unlocked(1), "Clean Pack 3 smoke profile should start with Level 1 unlocked."):
		return
	if not _require(not _profile.is_level_unlocked(2), "Clean Pack 3 smoke profile should start with Level 2 locked."):
		return
	if not _require(not _profile.is_level_unlocked(PACK_3_FIRST_LEVEL), "Clean Pack 3 smoke profile should start with Level 21 locked."):
		return
...
func _run_pack_3_completion_flow() -> void:
	for level in _pack_3_levels:
		...
		if level_number < PACK_3_LAST_LEVEL:
			...
			if not _require(int(_profile.data.get("unlocked_level", 0)) >= PACK_3_LAST_LEVEL + 1, "Completing Level 30 should advance Local Profile beyond Pack 3."):
				return
...
func _run_level_30_replay_best_attempt_checks() -> void:
	var level := _level_by_number(PACK_3_LAST_LEVEL)
	var level_id := str(level.get("id", ""))

	var first_best: Dictionary = _profile.get_best_attempt(level_id)
	if not _require(not first_best.is_empty(), "Level 30 should have a Best Attempt before replay."):
		return
	if not _require(int(first_best.get("roast_count", -1)) > 0, "Initial Level 30 completion should include Roast usage so replay can improve it."):
		return
	...
	if not _require(int(better_best.get("roast_count", -1)) == 0, "Better Level 30 replay should improve Best Attempt Roast count."):
		return
	if not _require(int(better_best.get("action_count", 9999)) <= int(first_best.get("action_count", 9999)), "Better Level 30 replay should keep or improve Best Attempt action count."):
		return
	...
	if not _require(int(kept_best.get("roast_count", -1)) == 0, "Worse Level 30 replay should keep the improved Best Attempt Roast count."):
		return
	if not _require(int(kept_best.get("action_count", 9999)) == int(better_best.get("action_count", 9999)), "Worse Level 30 replay should keep the improved Best Attempt action count."):
		return
...
func _complete_level_by_template(level: Dictionary) -> void:
	match str(level.get("template", "")):
		...
		_:
			if not _require(false, "Unsupported Level Template in Pack 3 smoke: %s" % str(level.get("template", ""))):
				return
...
func _assert_restart_state() -> void:
	...
	if not _require(_profile.current_uqiq_score() == expected_score, "App restart should preserve UQIQ Score after Pack 3."):
		return
	if not _require(_profile.dur_tokens() == expected_tokens, "App restart should preserve Dur Tokens after Pack 3."):
		return
	if not _require(int(_profile.data.get("unlocked_level", 0)) == expected_unlocked, "App restart should preserve Level 30 unlock progress."):
		return
...
func _level_by_number(level_number: int) -> Dictionary:
	...
	_require(false, "Level %d should be loaded for Issue #11 smoke." % level_number)
	return {}
