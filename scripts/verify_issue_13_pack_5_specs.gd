...
const PACK_PATH := "res://content/levels/pack_05_brain_buffer_full.json"
const EXPECTED_PACK_ID := "pack_05_brain_buffer_full"
const EXPECTED_PACK_TITLE := "Brain Buffer Full"
const EXPECTED_LEVEL_COUNT := 10
const FIRST_LEVEL_NUMBER := 41
const PRIMARY_TEMPLATE := "Memory Flash"
...
func _initialize() -> void:
	if not FileAccess.file_exists(PACK_PATH):
		_fail("Missing Pack 5 Level File: %s" % PACK_PATH)
		_finish()
		return

	var file := FileAccess.open(PACK_PATH, FileAccess.READ)
	if file == null:
		_fail("Could not open Pack 5 Level File: %s" % PACK_PATH)
		_finish()
		return

	var parsed = JSON.parse_string(file.get_as_text())
	if typeof(parsed) != TYPE_DICTIONARY:
		_fail("Pack 5 Level File must be a JSON object: %s" % PACK_PATH)
		_finish()
		return
...
func _validate_pack(pack: Dictionary) -> void:
	...
	if not pack.has("pack_id"):
		_fail("Pack 5 file missing top-level pack_id.")
	elif str(pack.get("pack_id", "")).strip_edges() != EXPECTED_PACK_ID:
		_fail("Pack 5 pack_id must be %s." % EXPECTED_PACK_ID)

	if not pack.has("pack_title"):
		_fail("Pack 5 file missing top-level pack_title.")
	elif str(pack.get("pack_title", "")).strip_edges() != EXPECTED_PACK_TITLE:
		_fail("Pack 5 pack_title must be %s." % EXPECTED_PACK_TITLE)

	var levels = pack.get("levels", [])
	if typeof(levels) != TYPE_ARRAY:
		_fail("Pack 5 file must contain a levels array.")
		return

	if levels.size() != EXPECTED_LEVEL_COUNT:
		_fail("Pack 5 must contain exactly %d Level Specs, found %d." % [EXPECTED_LEVEL_COUNT, levels.size()])
	...
	if primary_template_count < PRIMARY_TEMPLATE_MIN_LEVELS:
		_fail("Pack 5 must use %s as Primary Template: expected at least %d Levels, found %d." % [
			PRIMARY_TEMPLATE,
	...
	match typeof(value):
		...
		TYPE_STRING:
			if _looks_like_placeholder(str(value)):
				_fail("%s contains placeholder or future text." % path)
...
func _finish() -> void:
	if _errors.is_empty():
		print("Issue #13 Pack 5 spec verification passed: %d Level Specs, %s, %s, Levels %d-%d, supported templates, scoring, Roasts, unique ids, primary Memory Flash coverage, and UQIQ Moments." % [
	...
	for message in _errors:
		push_error(message)
	print("Issue #13 Pack 5 spec verification failed: %d error(s)." % _errors.size())
	quit(1)
